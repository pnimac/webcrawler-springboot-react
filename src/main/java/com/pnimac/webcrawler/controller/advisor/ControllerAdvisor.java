package com.pnimac.webcrawler.controller.advisor;
...
public class ControllerAdvisor extends ResponseEntityExceptionHandler {
	...
	@Autowired
	public ControllerAdvisor(OperationService operationService) {
	...
	public ResponseEntity<GenericResponse> handleException(MalformedURLException ex, WebRequest request) {
		...
		GenericResponse response = GenericResponse.builder().message(ex.getMessage())
				.reason(ex.getCause() == null ? new String[] { ex.getLocalizedMessage() }
	...
	public ResponseEntity<GenericResponse> handleException(BackendException ex, WebRequest request) {
		...
		GenericResponse response = GenericResponse.builder().message(ex.getMessage())
				.reason(ex.getCause() == null ? new String[] { ex.getLocalizedMessage() }
						: new String[] { ex.getCause().getClass().getSimpleName() })
				.code(5010).build();
		handleSavingOperation(request);
	...
	public ResponseEntity<GenericResponse> handleException(NotFoundException ex, WebRequest request) {
		log.error("error {}", ex.getMessage());
		GenericResponse response = GenericResponse.builder().message(ex.getMessage()).code(4004).build();
		handleSavingOperation(request);
		return ResponseEntity.badRequest().body(response);
	}
	...
	public void handleSavingOperation(WebRequest request) {
		...
		try {
			Operation operation = Operation.builder().info(mapper.writeValueAsString(request.getDescription(true)))
					.status(OperationStatus.FAILURE).build();
